    ...
    class Display
    {
        ...
        public static double CentreFreqRX1
        {
            ...
            set
            {
                ...
                {
                    // SpecHPSDRDLL.SetPixelRef(cmaster.inid(0, 0), value); // removed in WDSP 2.00
        ...
        public static double CentreFreqRX2
        {
            ...
            set
            {
                ...
                {
                    // SpecHPSDRDLL.SetPixelRef(cmaster.inid(0, 1), value); // removed in WDSP 2.00
                    ...
                    if (_waterfall_bmp_dx2d != null && !_waterfall_bmp_dx2d.IsDisposed)
                    {
                        if (displayTargetWidth == _waterfall_bmp_dx2d.Size.Width)
                        {
                            ...
                            tmp = new SharpDX.Direct2D1.Bitmap(_d2dRenderTarget, new Size2((int)_waterfall_bmp_dx2d.Size.Width, h),
                                    new BitmapProperties(new SDXPixelFormat(WaterfallPixelWriter.DxgiFormat, ALPHA_MODE))); // Yurij-eu2av - 2026-07-04: active depth
        ...
        // Yurij-eu2av - 2026-07-04: Direct2D 1.1 migration — Factory1 + Device +
        // target bitmap replace the legacy RenderTarget-from-surface path so the
        // swap chain can use 10/16-bit back-buffer formats (R10G10B10A2 / R16G16B16A16).
        // _d2dRenderTarget stays typed as RenderTarget so the ~96 drawing calls
        // (DrawBitmap, DrawText, BeginDraw, …) work unchanged via polymorphism.
        private static SharpDX.Direct2D1.Factory1 _d2dFactory;
        private static SharpDX.Direct2D1.Device _d2dDevice;
        private static SharpDX.Direct2D1.Bitmap1 _d2dTargetBitmap;
        ...
        public static void ShutdownDX2D()
        {
            lock (_objDX2Lock)
            {
                ...
                try
                {
                    ...
                    // Yurij-eu2av - 2026-07-04: detach the back-buffer bitmap from
                    // the DeviceContext and dispose the D2D 1.1 objects added by
                    // the migration before disposing the render target itself.
                    if (_d2dRenderTarget is SharpDX.Direct2D1.DeviceContext shutdownCtx)
                        shutdownCtx.Target = null;
        ...
        // Yurij-eu2av - 2026-07-04: Live color-depth switch (no restart required).
        // Tears down the swap chain + D2D bitmaps and rebuilds them with the
        // newly-selected surface format (8/10/16-bit). Called from the Setup
        // "Depth" combo handler. Falls back to 8-bit if the new format is not
        // supported as a render target on this GPU.
        //
        // NOTE: ShutdownDX2D and initDX2D each take _objDX2Lock internally, so
        // this method must NOT hold the lock across both — that would deadlock.
        // Between the two calls the render thread sees _bDX2Setup=false and
        // safely no-ops (RenderDX2D checks that flag inside its own lock).
        //
        // Returns true on success; on failure it reverts to 8-bit and retries,
        // so the display never stays in a broken state.
        ...
        // Yurij-eu2av - 2026-07-04: create a Direct2D 1.1 DeviceContext bound to
        // the current swap-chain back buffer (_surface). Replaces the legacy
        // `new RenderTarget(_d2dFactory, _surface, rtp)` path which only supports
        // 8-bit surfaces. DeviceContext supports 10/16-bit back-buffer formats.
        //
        // Pre: _device (D3D11, BgraSupport), _d2dFactory (Factory1), _surface
        //      (swap-chain back buffer) must all be valid.
        //
        // IMPORTANT: the D2D Device is created ONCE and kept across resizes.
        // Recreating it would invalidate all existing D2D bitmaps (waterfall,
        // background, fonts) with D2DERR_WRONG_RESOURCE_DOMAIN, because each
        // bitmap is tied to the device that created it. Only the DeviceContext
        // and the back-buffer target bitmap are refreshed here.
        //
        // FALLBACK: if DeviceContext creation fails, we fall back to the legacy
        // 8-bit RenderTarget and write the cause to d2d_diag.txt.
        private static void createD2DRenderTarget()
        {
            // Detach + dispose the previous DeviceContext and its target bitmap,
            // but KEEP _d2dDevice (bitmaps created on it must survive resizes).
            if (_d2dRenderTarget is SharpDX.Direct2D1.DeviceContext prevCtx)
                prevCtx.Target = null;
            Utilities.Dispose(ref _d2dTargetBitmap);
            Utilities.Dispose(ref _d2dRenderTarget);
            _d2dTargetBitmap = null;
            ...
            SharpDX.DXGI.Format surfaceFmt = SharpDX.DXGI.Format.Unknown;
            try { surfaceFmt = _surface.Description.Format; } catch { }

            try
            {
                // 1. Create the D2D device ONCE. On subsequent calls (resize) the
                //    existing device is reused so waterfall/background bitmaps
                //    remain valid. Created from the DXGI device of the D3D11 device.
                if (_d2dDevice == null)
                {
                    SharpDX.DXGI.Device dxgiDevice = _device.QueryInterface<SharpDX.DXGI.Device>();
                    try { _d2dDevice = new SharpDX.Direct2D1.Device(_d2dFactory, dxgiDevice); }
                    finally { Utilities.Dispose(ref dxgiDevice); }
                }

                // 2. Create a fresh DeviceContext (cheap; owns no resources).
                SharpDX.Direct2D1.DeviceContext ctx = new SharpDX.Direct2D1.DeviceContext(
                    _d2dDevice, SharpDX.Direct2D1.DeviceContextOptions.None);
                _d2dRenderTarget = ctx;

                // 3. Wrap the swap-chain back buffer as a D2D Bitmap1 flagged as a
                //    render target. Format.Unknown lets D2D inherit the surface's
                //    format; Alpha is Ignore because back buffers are opaque.
                SharpDX.Direct2D1.BitmapProperties1 bp = new SharpDX.Direct2D1.BitmapProperties1(
                    new SDXPixelFormat(SharpDX.DXGI.Format.Unknown, AlphaMode.Ignore),
                    96f, 96f,
                    SharpDX.Direct2D1.BitmapOptions.Target | SharpDX.Direct2D1.BitmapOptions.CannotDraw);

                _d2dTargetBitmap = new SharpDX.Direct2D1.Bitmap1(ctx, _surface, bp);
                ...
                // default differs from the legacy RenderTarget (which is 96), lines
                // render at a different thickness and text at a different size.
                // Pinning to 96 matches the legacy behaviour exactly.
                ctx.DotsPerInch = new SharpDX.Size2F(96f, 96f);
            }
            catch (Exception ex)
            {
                // Yurij-eu2av - 2026-07-04: DeviceContext path failed. Fall back to
                // the legacy RenderTarget so the app always starts. Use the surface's
                // ACTUAL format (not a hardcoded one) so the fallback can wrap the
                // current swap-chain back buffer whatever format it was created at.
                ...
                SharpDX.DXGI.Format fallbackFmt = surfaceFmt;
                if (fallbackFmt == SharpDX.DXGI.Format.Unknown)
                    fallbackFmt = SharpDX.DXGI.Format.B8G8R8A8_UNorm;

                RenderTargetProperties rtp = new RenderTargetProperties(
                    new SDXPixelFormat(fallbackFmt, ALPHA_MODE));
        ...
        private static void initDX2D(DriverType driverType = DriverType.Hardware, AdaptorInfo adaptorInfo = null)
        {
            lock (_objDX2Lock)
            {
                ...
                try
                {
                    ...
                    // Yurij-eu2av - 2026-07-04: sync PixelSize/DxgiFormat to the
                    // active depth. We do NOT pre-check with CheckFormatSupport here:
                    // it reports generic RenderTarget support which does not match
                    // the set of formats valid as a swap-chain BACK BUFFER (e.g.
                    // R10G10B10A2_UNorm is reported as unsupported on some drivers
                    // even though it works fine as a flip-model back buffer). If the
                    // format truly cannot be used, new SwapChain() below throws and
                    // the catch block shuts down DX; RebuildForColorDepth() then
                    // retries with 8-bit.
                    WaterfallPixelWriter.UpdateFormat();
                    ...
                    ModeDescription md = new ModeDescription(displayTarget.Width, displayTarget.Height,
                                                               new Rational(console.DisplayFPS, 1),
                                                               WaterfallPixelWriter.DxgiFormat); // Yurij-eu2av - 2026-07-04: active depth (8/10/16-bit). Was Format.B8G8R8A8_UNorm
                    ...
                    // build a Device + DeviceContext; the legacy Factory (1.0) cannot.
                    _d2dFactory = new SharpDX.Direct2D1.Factory1(FactoryType.SingleThreaded, DebugLevel.None);

                    _surface = _swapChain1.GetBackBuffer<Surface>(0);

                    // Yurij-eu2av - 2026-07-04: create DeviceContext bound to the back
            ...
            try
            {
                lock (_objDX2Lock)
                {
                    ...
                    ModeDescription modeDesc = new ModeDescription(displayTargetWidth, displayTargetHeight,
                                                       new Rational(console.DisplayFPS, 1),
                                                       WaterfallPixelWriter.DxgiFormat); // Yurij-eu2av - 2026-07-04: active depth
        ...
        private static void pauseDisplay()
        {
            ...
            if (_paused_display)
            {
                ...
                SharpDX.Direct2D1.PixelFormat pixelFormat = new SharpDX.Direct2D1.PixelFormat(WaterfallPixelWriter.DxgiFormat, ALPHA_MODE); // Yurij-eu2av - 2026-07-04: active depth
                ...
                for (int i = 0; i < nDecimatedWidth; i++)
                {
                    ...
                    if (bSpectralPeakHold)
                    {
                        ...
                        if (peak.max_dBm >= max)
                        {
                            ...
                            if (bActivePeakFill)
                            {
                                _d2dRenderTarget.DrawLine(point, spectralPeakPoint, fillPeaksBrush, sw(local_Decimation));
                            }
                            else
                            {
                                _d2dRenderTarget.DrawLine(oldSpectralPeakPoint, spectralPeakPoint, fillPeaksBrush, sw(line_width));
                    ...
                    if (bIgnoringPoints)
                    {
                        _d2dRenderTarget.DrawLine(previousPoint, lastIgnoredPoint, lineBrush, sw(line_width));
                        previousPoint = lastIgnoredPoint;
                        bIgnoringPoints = false;
                    }
                    _d2dRenderTarget.DrawLine(previousPoint, point, lineBrush, sw(line_width));
                    previousPoint = point;
                }
        ...
        private static void clearWaterfallBitmapRegion(SharpDX.Direct2D1.Bitmap bitmap, int x, int y, int width, int height)
        {
            ...
            // Yurij-eu2av - 2026-07-04: pixel size follows the active depth
            // (4 for 8/10-bit, 8 for 16-bit). Was hardcoded const int pixelSize = 4.
            ...
            try
            {
                // FillClearBuffer writes opaque-black in the active depth's layout
                // (BGRA / R10G10B10A2 / R16G16B16A16_FLOAT). Was: Array.Clear + A=255.
        ...
        // Yurij-eu2av - 2026-07-04: shared renderer for the Console/Thermal/DeepBlue
        // gradient schemes. Looks up each pixel's signal level in the palette's
        // precomputed 256-entry float LUT and writes the result into rowF. Runs in
        // the same float pipeline as the legacy schemes, so Quality (S-curve),
        // gamma and dither all apply afterwards in the unified post-processing pass.
        //
        // Signature mirrors the locals available inside DrawWaterfallDX2D's colour
        // switch: waterfall signal, low/high thresholds, decimation, and the float
        // row buffer. waterfall_minimum is updated for AGC tracking.
        private static void DrawPaletteScheme(
            WaterfallPalette palette,
            float[] waterfall_data, float[] dataCopy, float fOffset,
            ref float waterfall_minimum,
            float low_threshold, float high_threshold,
            int nDecimatedWidth, int m_nDecimation,
            float[] rowF, float alphaF)
        {
            float range = high_threshold - low_threshold;
            for (int i = 0; i < nDecimatedWidth; i++)
            {
                float v = waterfall_data[i];
                float percent;
                if (v <= low_threshold)
                    percent = 0f;
                    ...
                    percent = (v - low_threshold) / range;

                // Quality S-curve shapes where the signal lands in the gradient,
                // giving the new schemes the same Vivid/Sharp/Ultra boost.
                percent = WaterfallEnhancer.ApplyQualityPercent(percent);

                palette.Sample(percent, out float R, out float G, out float B);

                if (waterfall_minimum > dataCopy[i] + fOffset)
                    waterfall_minimum = dataCopy[i] + fOffset;

                int fidx = (i * m_nDecimation) * 4;
                rowF[fidx + 0] = R;
                rowF[fidx + 1] = G;
                rowF[fidx + 2] = B;
                rowF[fidx + 3] = alphaF;
        ...
        // Yurij-eu2av - 2026-07-04: rotating row index for 2D Bayer dither coverage
        // (waterfall scrolls vertically each frame, so the same screen row sees a
        // different source row each time — using the frame counter spreads the
        // 8x8 pattern in both X and Y instead of only X).
        private static int _ditherFrameY = 0;

        // Yurij-eu2av - 2026-07-04: palette arrays now sized dynamically via
        // WaterfallEnhancer.LUT_SIZE (default 101 = classic behaviour).
        private static Color[] _rx1_waterfall_grad = new Color[WaterfallEnhancer.LUT_SIZE];
        private static Color[] _rx2_waterfall_grad = new Color[WaterfallEnhancer.LUT_SIZE];
        private static bool _rx1_waterfall_grad_ok = false;
        private static bool _rx2_waterfall_grad_ok = false;
        private static Color[] _tx_waterfall_grad = new Color[WaterfallEnhancer.LUT_SIZE];

        // Yurij-eu2av - 2026-07-04: multi-stop gradient palettes for the new
        // Console/Thermal/DeepBlue schemes. Precomputed 256-entry float LUTs;
        // built once at first use (lazy) so there is no startup cost if the
        // user never selects these schemes.
        ...
        {
            // Yurij-eu2av - 2026-07-04: accept any LUT size (101..1024)
            if (colours.Length < 2) return;

            int lut = colours.Length;

            Color[] cols;
            if (rx == 1)
            {
                _rx1_waterfall_grad_ok = false;
                // reallocate if palette resolution changed
                if (_rx1_waterfall_grad == null || _rx1_waterfall_grad.Length != lut)
                    _rx1_waterfall_grad = new Color[lut];
                cols = _rx1_waterfall_grad;
            }
            else if (rx == 2)
            {
                _rx2_waterfall_grad_ok = false;
                if (_rx2_waterfall_grad == null || _rx2_waterfall_grad.Length != lut)
                    _rx2_waterfall_grad = new Color[lut];
                cols = _rx2_waterfall_grad;
            }
            else
                return;

            for (int perc = 0; perc < lut; perc++)
            {
        ...
        private static void OnWaterfallTXGradientChanged(Color[] colours)
        {
            ...
            if (_tx_waterfall_grad == null || _tx_waterfall_grad.Length != lut)
                _tx_waterfall_grad = new Color[lut];
        ...
        unsafe static private bool DrawWaterfallDX2D(int nVerticalShift, int W, int H, int rx, bool bottom)
        {
            ...
            // Yurij-eu2av - 2026-07-04: waterfall colour pipeline now works in
            // float (0..255) and is quantised to the active depth at the very
            // end via WaterfallPixelWriter.EncodeRow. Was: int R,G,B (8-bit).
            ...
            if (console.PowerOn)
            {
                ...
                if (bRXdraw)
                {
                    ...
                    // Yurij-eu2av - 2026-07-04: float colour pipeline. The row is
                    // built in float (rowF, RGBA, W*4) and encoded to the active
                    // depth's byte layout (row) by WaterfallPixelWriter just before
                    // CopyFromMemory. pixel_size follows the output format (4 or 8).
                    float alphaF = 255f;
                    int pixel_size = WaterfallPixelWriter.PixelSize;
                    float[] rowF = new float[W * 4];        // RGBA float, 0..255
                    byte[] row = new byte[W * pixel_size];  // packed output (BGRA/R10G10B10A2/half)
                    ...
                    #region colours
                    // Yurij-eu2av - 2026-07-04: if a scheme cannot produce colour
                    // (e.g. Custom with no gradient loaded), it sets skipRow=true so
                    // the rest of the pipeline (post-process, EncodeRow, CopyFromMemory)
                    // is skipped entirely. Without this, an all-zero (black) row was
                    // uploaded and slowly overwrote the existing waterfall — the
                    // long-standing "dim waterfall after Custom" bug.
                    bool skipRow = false;
                    switch (cScheme)
                    {
                        case (ColorScheme.Custom):
                            {
                                ...
                                if (local_mox)
                                {
                                    if (!_tx_waterfall_grad_ok) { skipRow = true; break; }
                                ...
                                else
                                {
                                    if (rx == 1)
                                    {
                                        if (!_rx1_waterfall_grad_ok) { skipRow = true; break; }
                                        cols = _rx1_waterfall_grad;
                                    }
                                    else
                                    {
                                        if (!_rx2_waterfall_grad_ok) { skipRow = true; break; }
                                        cols = _rx2_waterfall_grad;
                                    }
                                }

                                int lut = cols.Length;   // Yurij-eu2av - 2026-07-04: dynamic LUT size
                                ...
                                for (int i = 0; i < nDecimatedWidth; i++)   // for each pixel in the new line
                                {
                                    ...
                                    {
                                        R = cols[lut - 1].R;   // Yurij-eu2av - 2026-07-04: was cols[100]
                                        G = cols[lut - 1].G;
                                        B = cols[lut - 1].B;
                                    }
                                    else // value is between low and high
                                    {
                                        ...
                                        // Yurij-eu2av - 2026-07-04: Quality S-curve + gamma + LUT index
                                        overall_percent = WaterfallEnhancer.ApplyQualityPercent(overall_percent);
                                        overall_percent = WaterfallEnhancer.ApplyGamma(overall_percent);
                                        int perc = (int)(overall_percent * (lut - 1));
                                        if (perc < 0) perc = 0;
                                        else if (perc >= lut) perc = lut - 1;
                                    ...
                                    // Yurij-eu2av - 2026-07-04: dither/gamma now applied
                                    // once, in float, in the unified post-process pass
                                    // below (works for ALL schemes). The old in-loop
                                    // byte dither here was redundant and double-applied.

                                    if (waterfall_minimum > dataCopy[i] + fOffset) //[2.10.3]MW0LGE use non notched data
                                        waterfall_minimum = dataCopy[i] + fOffset;

                                    // Yurij-eu2av - 2026-07-04: write float RGBA (unified order)
                                    int fidx = (i * m_nDecimation) * 4;
                                    rowF[fidx + 0] = R;
                                    rowF[fidx + 1] = G;
                                    rowF[fidx + 2] = B;
                        ...
                        case (ColorScheme.enhanced):
                            {
                                ...
                                for (int i = 0; i < nDecimatedWidth; i++)   // for each pixel in the new line
                                {
                                    ...
                                    else // value is between low and high
                                    {
                                        ...
                                        float offset = waterfall_data[i] - low_threshold;
                                        float overall_percent = offset / range; // value from 0.0 to 1.0 where 1.0 is high and 0.0 is low.
                                        // Yurij-eu2av - 2026-07-04: Quality S-curve shapes where
                                        // the signal lands in the palette — visible on enhanced too.
                                        overall_percent = WaterfallEnhancer.ApplyQualityPercent(overall_percent);

                                        if (overall_percent < (float)2 / 9) // background to blue
                                        {
                                            float local_percent = overall_percent / ((float)2 / 9);
                                            R = (float)((1.0 - local_percent) * low_color.R);
                                            G = (float)((1.0 - local_percent) * low_color.G);
                                            B = (float)(low_color.B + local_percent * (255 - low_color.B));
                                        }
                                        else if (overall_percent < (float)3 / 9) // blue to blue-green
                                        {
                                            float local_percent = (overall_percent - (float)2 / 9) / ((float)1 / 9);
                                            R = 0f;
                                            G = local_percent * 255f;
                                            B = 255f;
                                        }
                                        ...
                                        else if (overall_percent < (float)8 / 9) // red to red-blue
                                        {
                                            ...
                                            R = 255f;
                                            G = 0f;
                                            B = local_percent * 255f;
                                        }
                                        else // red-blue to purple end
                                        {
                                            float local_percent = (overall_percent - (float)8 / 9) / ((float)1 / 9);
                                            R = (float)((0.75 + 0.25 * (1.0 - local_percent)) * 255);
                                            G = local_percent * 255f * 0.5f;
                                            B = 255f;
                                        }
                                    }
                        ...
                        case (ColorScheme.SPECTRAN):
                            {
                                ...
                                for (int i = 0; i < nDecimatedWidth; i++)   // for each pixel in the new line
                                {
                                    ...
                                    else // value is between low and high
                                    {
                                        ...
                                        // Yurij-eu2av - 2026-07-04: Quality S-curve on the signal
                                        // percentage (0..100) so contrast is visible here too.
                                        local_percent = WaterfallEnhancer.ApplyQualityPercent(local_percent / 100f) * 100f;

                                        // Yurij-eu2av - 2026-07-04: removed (int) casts so the
                                        // B/R/G values stay in float (sub-LSB precision kept until
                                        // quantisation in WaterfallPixelWriter).
                                        ...
                                        {
                                            R = G = (local_percent - 50f) * 2f;
                                            B = 255f;
                                        }
                                        else if (local_percent < 77.0f)
                                        {
                                            R = G = (local_percent - 50f) * 3f;
                                            B = 255f;
                                        }
                                        else if (local_percent < 88.0f)
                                        {
                                            R = G = (local_percent - 50f) * 4f;
                                            B = 255f;
                                        }
                                        else if (local_percent < 99.0f)
                                        {
                                            R = G = (local_percent - 50f) * 5f;
                                            B = 255f;
                                        }
                                    }
                        ...
                        case (ColorScheme.BLACKWHITE):
                            {
                                ...
                                for (int i = 0; i < nDecimatedWidth; i++)   // for each pixel in the new line
                                {
                                    ...
                                    else // value is between low and high
                                    {
                                        ...
                                        // percentage (0..100) so contrast is visible here too.
                                        local_percent = WaterfallEnhancer.ApplyQualityPercent(local_percent / 100f) * 100f;
                                        R = (local_percent / 100f) * 255f;  // Yurij-eu2av - 2026-07-04: float, was (int)
                                        G = R;
                                        B = R;
                                    }
                        ...
                        case (ColorScheme.LinLog):
                            {
                                for (int i = 0; i < nDecimatedWidth; i++)   // for each pixel in the new line
                                {
                                    ...
                                    // Yurij-eu2av - 2026-07-04: write float RGBA (unified order).
                                    // LinLog/LinRad/LinAuto previously wrote RGBA byte order
                                    // (reverse of the BGRA used elsewhere) — see #289. With the
                                    // unified float RGBA pipeline this quirk is resolved: all
                                    // schemes now emit the same channel order.
                                    {
                                        int fidx = (i * m_nDecimation) * 4;
                                        rowF[fidx + 0] = R;
                                        rowF[fidx + 1] = G;
                        ...
                        case (ColorScheme.LinRad):
                            {
                                for (int i = 0; i < nDecimatedWidth; i++)   // for each pixel in the new line
                                {
                                    ...
                                    // Yurij-eu2av - 2026-07-04: write float RGBA (unified order;
                                    // resolves the LinRad RGBA-vs-BGRA quirk from #289).
                                    {
                                        int fidx = (i * m_nDecimation) * 4;
                                        rowF[fidx + 0] = R;
                                        rowF[fidx + 1] = G;
                        ...
                        // Yurij-eu2av - 2026-07-04: new multi-stop gradient schemes.
                        // These use a precomputed 256-entry float LUT (WaterfallPalette)
                        // instead of hard if/else colour bands. The result is a smooth,
                        // saturated gradient like SDR Console / Spectrum Lab.
                        case (ColorScheme.Console):
                            DrawPaletteScheme(GetPaletteConsole(), waterfall_data, dataCopy, fOffset,
                                ref waterfall_minimum, low_threshold, high_threshold,
                                nDecimatedWidth, m_nDecimation, rowF, alphaF);
                            break;
                        case (ColorScheme.Thermal):
                            DrawPaletteScheme(GetPaletteThermal(), waterfall_data, dataCopy, fOffset,
                                ref waterfall_minimum, low_threshold, high_threshold,
                                nDecimatedWidth, m_nDecimation, rowF, alphaF);
                            break;
                    ...
                    // Yurij-eu2av - 2026-07-04: skip the row-write pipeline entirely
                    // when the active scheme could not produce colour (e.g. Custom
                    // with no gradient loaded). This was the root cause of the
                    // long-standing "dim waterfall after selecting Custom" bug: the
                    // all-zero rowF was quantised + uploaded, slowly overwriting the
                    // existing waterfall with black. Skipping keeps the previous row.
                    if (!skipRow)
                    {
                    // Yurij-eu2av - 2026-07-04: fill decimation gaps in the FLOAT row
                    // (was: byte row). Copies the computed pixel to its m_nDecimation
                    // neighbours so there are no transparent gaps when decimating.
                    for (int i = 0; i < nDecimatedWidth; i++)
                    {
                        int src = (i * m_nDecimation) * 4;
                        for (int j = 1; j < m_nDecimation; j++)
                        {
                            int dst = ((i * m_nDecimation) + j) * 4;
        ...
        // Yurij-eu2av - 2026-07-04: convert System.Drawing.Color to SharpDX Color4.
        // On 16-bit float surfaces D2D interprets values as LINEAR RGB, but the
        // source colours are sRGB — so we apply the sRGB→linear transfer here to
        // keep the on-screen appearance identical to the 8-bit path.
        //
        // Alpha is also compensated: on a linear surface, alpha-blended coverage
        // (e.g. the RX filter overlay) composites differently and renders too
        // transparent. We square the alpha toward the linear curve so a value
        // that looked right at 8-bit (e.g. 0.26) keeps the same visual opacity
        // on 16-bit. Fully opaque (1.0) is unchanged.
        private static Color4 convertColour(Color c)
        {
            ...
            float b = c.B / 255f;
            float a = c.A / 255f;
            if (WaterfallPixelWriter.PixelSize == 8) // 16-bit float active
            {
                r = WaterfallPixelWriter.SrgbToLinear(r);
                g = WaterfallPixelWriter.SrgbToLinear(g);
                b = WaterfallPixelWriter.SrgbToLinear(b);
        ...
        // Yurij-eu2av - 2026-07-04: convert a System.Drawing.Bitmap (8-bit BGRA,
        // premultiplied alpha) into a D2D bitmap. The bitmap is always created
        // at the ACTIVE swap-chain format so DrawBitmap does no implicit format
        // conversion. On 16-bit float that means each 8-bit sRGB source channel
        // is expanded to a linear-RGB half-float; without this, a photo
        // background renders visibly different (brighter/washed-out) on 16-bit.
        private static SharpDX.Direct2D1.Bitmap SDXBitmapFromSysBitmap(RenderTarget rt, System.Drawing.Bitmap bitmap)
        {
            ...
            // Lock System.Drawing.Bitmap as 32bpp premultiplied ARGB.
            ...
            SharpDX.Direct2D1.Bitmap dx2bitmap;
            int pixelCount = bitmap.Width * bitmap.Height;

            if (WaterfallPixelWriter.PixelSize == 8) // 16-bit float (R16G16B16A16_Float)
            {
                // Expand 8-bit sRGB → linear-RGB half-float, 8 bytes/pixel.
                int stride = bitmap.Width * 8;
                ...
                for (int y = 0; y < bitmap.Height; y++)
                {
                    ...
                    for (int x = 0; x < bitmap.Width; x++)
                    {
                        ...
                        // Yurij-eu2av - 2026-07-04: sRGB → linear so D2D's LINEAR
                        // interpretation of the R16G16B16A16_Float surface matches
                        // what 8-bit sRGB showed. RGBA order (not BGRA).
                        float lr = WaterfallPixelWriter.SrgbToLinear(R / 255f);
                        float lg = WaterfallPixelWriter.SrgbToLinear(G / 255f);
                        float lb = WaterfallPixelWriter.SrgbToLinear(B / 255f);
                        float la = A / 255f;
                        WriteHalf(tempStream, lr);
                        WriteHalf(tempStream, lg);
                        WriteHalf(tempStream, lb);
                        WriteHalf(tempStream, la);
                    }
                }
                bitmap.UnlockBits(bitmapData);
                tempStream.Position = 0;

                BitmapProperties bp = new BitmapProperties(new SDXPixelFormat(
                    WaterfallPixelWriter.DxgiFormat, ALPHA_MODE));
                dx2bitmap = new SharpDX.Direct2D1.Bitmap(rt, size, tempStream, stride, bp);
                Utilities.Dispose(ref tempStream);
            }
            else
            {
                // 8-bit path: original BGRA packing.
        ...
        // Yurij-eu2av - 2026-07-04: half-float packing helper for background images
        // on 16-bit surfaces. The sRGB→linear transfer lives in WaterfallPixelWriter.
        private static void WriteHalf(DataStream s, float value)
        {
            ushort bits = WaterfallPixelWriter.FloatToHalfBitsPublic(value);
        ...
        unsafe static private bool DrawSpectrumDX2D(int rx, int W, int H, bool bottom)
        {
            ...
            for (int i = 0; i < nDecimatedWidth; i++)
            {
                ...
                _d2dRenderTarget.DrawLine(previousPoint, point, m_bDX2_data_line_pen_brush, sw(data_line_pen.Width));
        ...
        unsafe private static bool DrawScopeDX2D(int W, int H, bool bottom)
        {
            ...
            for (int i = 1; i < nDecimatedWidth; i++)
            {
                ...
                if (previousPointMax.Y > pointMin.Y) _d2dRenderTarget.DrawLine(previousPointMax, pointMin, m_bDX2_waveform_line_pen, sw(1f));
                if (previousPointMin.Y < pointMax.Y) _d2dRenderTarget.DrawLine(previousPointMin, pointMax, m_bDX2_waveform_line_pen, sw(1f));

                if (pointMin == pointMax)
                    _d2dRenderTarget.FillRectangle(new RectangleF(pointMin.X, pointMin.Y, 1f, 1f), m_bDX2_waveform_line_pen);
                else
                    _d2dRenderTarget.DrawLine(pointMin, pointMax, m_bDX2_waveform_line_pen, sw(1f));
        ...
        unsafe private static bool DrawScope2DX2D(int W, int H, bool bottom)
        {
            ...
            for (int x = 0; x < nDecimatedWidth; x++)
            {
                ...
                _d2dRenderTarget.DrawLine(previousPoint, point, m_bDX2_waveform_line_pen, sw(1f));
        ...
        unsafe private static bool DrawPhaseDX2D(int W, int H, bool bottom)
        {
            ...
            if (m_bShowPhaseAngularMean)
            {
                ...
                _d2dRenderTarget.DrawLine(pMiddle, pEnd, m_bDX2_Red, sw(3f));
            }
            //
        ...
        // Yurij-eu2av - 2026-07-04: DPI query helper (temp debug)
